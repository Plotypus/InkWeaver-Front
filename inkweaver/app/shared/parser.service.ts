...
export class ParserService {
    ...
    public parseContent(paragraphs: Paragraph[], aliasTable: AliasTable, linkTable: LinkTable, passiveLinkTable: PassiveLinkTable): [ContentObject, ID] {
    ...
    public parseHtml(paragraphs: any): ContentObject {
        ...
        for (let paragraph of paragraphs) {
            let newText: string = paragraph.innerHTML.replace(new RegExp("<code>(?!.*?</code>).*?</code>"), '');
            let p: Paragraph = {
                paragraph_id: new ID(),
