...
const story_model_1 = require('../models/story/story.model');
const section_model_1 = require('../models/story/section.model');
const tooltip_model_1 = require('../models/story/tooltip.model');
const content_object_model_1 = require('../models/story/content-object.model');
const wiki_summary_model_1 = require('../models/wiki/wiki-summary.model');
const wiki_model_1 = require('../models/wiki/wiki.model');
const segment_model_1 = require('../models/wiki/segment.model');
const page_model_1 = require('../models/wiki/page.model');
const stats_model_1 = require('../models/stats/stats.model');
let ApiService = class ApiService {
    ...
    connect() {
        let url = this.local ? 'ws://localhost:8080/ws' : 'wss://inkweaver.plotypus.net:8080/ws';
        this.messages = this.socket.connect(url)
            .map((res) => {
            ...
            if (reply) {
                ...
                if (identifier) {
                    ...
                    let message_id = identifier.message_id;
                ...
                if (reply.event) {
                    switch (reply.event) {
                        ...
                        case 'story_collaborator_removed':
                            let index = this.data.collaborators.findIndex((collaborator) => {
                        ...
                        case 'story_collaborator_status_revoked':
                            let idx = this.data.stories.findIndex((story) => {
                        ...
                        case 'story_updated':
                            ...
                            let updateIdx = this.data.stories.findIndex((story) => {
                        ...
                        case 'unsubscribed_story_deleted':
                            let delIdx = this.data.stories.findIndex((story) => {
                                return JSON.stringify(story.story_id) === JSON.stringify(reply.story_id);
                            });
                        ...
                        case 'got_section_content':
                            if (!metadata.pdf) {
                                ...
                                this.data.section = this.parser.setSection(this.data.storyNode[0], JSON.stringify(metadata.section_id));
                                this.data.prevSection = this.data.section;
                        ...
                        case 'inner_subsection_added':
                            this.parser.findSection(this.data.storyNode[0], JSON.stringify(reply.parent_id), (found) => {
                                let newSection = {
                        ...
                        case 'section_title_updated':
                            this.parser.findSection(this.data.storyNode[0], JSON.stringify(reply.section_id), (found) => {
                                let old = found.data.title;
                                ...
                                if (this.data.storyDisplay && this.data.section.data && JSON.stringify(this.data.section.data.section_id) === JSON.stringify(found.data.section_id)) {
                                    this.data.storyDisplay = this.data.storyDisplay.replace('<h1>' + old + '</h1>', '<h1>' + reply.new_title + '</h1>');
                                }
                            });
                            break;
                        case 'section_deleted':
                            this.parser.findSection(this.data.storyNode[0], JSON.stringify(reply.section_id), (found) => {
                                let index = found.parent.children.indexOf(found);
                        ...
                        case 'subsection_moved_as_inner':
                            let moved = this.parser.findSection(this.data.storyNode[0], JSON.stringify(reply.section_id), (found) => {
                                let index = found.parent.children.indexOf(found);
                                found.parent.children.splice(index, 1);
                            });
                            this.parser.findSection(this.data.storyNode[0], JSON.stringify(reply.to_parent_id), (found) => {
                                moved.parent = found;
                        ...
                        case 'paragraph_added':
                            if (this.data.storyDisplay && this.data.section.data && JSON.stringify(reply.section_id) === JSON.stringify(this.data.section.data.section_id)) {
                                let p = {
                                    ...
                                    preceding_paragraph_id: new id_model_1.ID(), succeeding_paragraph_id: reply.succeeding_paragraph_id,
                                    links: new alias_table_model_1.AliasTable(), passiveLinks: new alias_table_model_1.AliasTable()
                                };
                                ...
                                let preKey = new id_model_1.ID();
                                let postKey = new id_model_1.ID();
                                ...
                                let pString = this.parser.setParagraph(p);
                                this.data.story.position_context = { 'cursor': p.paragraph_id.$oid };
                                if (myMessage) {
                                    let regex = new RegExp('<p id="added">(.*?)</p>');
                                    ...
                                    if (preKey.$oid) {
                                        let regex = new RegExp('<p id="' + preKey.$oid + '">.*?</p>');
                                        this.data.storyDisplay = this.data.storyDisplay.replace(regex, '$&' + pString);
                                    }
                                    else if (postKey.$oid) {
                                        let regex = new RegExp('<p id="' + postKey.$oid + '">.*?</p>');
                                        this.data.storyDisplay = this.data.storyDisplay.replace(regex, pString + '$&');
                                    }
                                    else {
                        ...
                        case 'paragraph_updated':
                            if (this.data.storyDisplay && this.data.section.data && JSON.stringify(reply.section_id) === JSON.stringify(this.data.section.data.section_id)) {
                                let p = this.data.contentObject[JSON.stringify(reply.paragraph_id)];
                                p.text = reply.update.text;
                                this.parser.parseParagraph(p, this.data.aliasTable, this.data.linkTable, this.data.passiveLinkTable);
                                let pString = this.parser.setParagraph(p);
                                this.data.story.position_context = { 'cursor': p.paragraph_id.$oid };
                                let regex = new RegExp('<p id="' + p.paragraph_id.$oid + '">.*?</p>');
                                this.data.storyDisplay = this.data.storyDisplay.replace(regex, pString);
                            }
                            break;
                        case 'paragraph_deleted':
                            if (this.data.storyDisplay && this.data.section.data && JSON.stringify(reply.section_id) === JSON.stringify(this.data.section.data.section_id)) {
                                if (this.data.contentObject[JSON.stringify(reply.paragraph_id)]) {
                                    let p = this.data.contentObject[JSON.stringify(reply.paragraph_id)];
                                    let preKey = p.preceding_paragraph_id;
                                    let postKey = p.succeeding_paragraph_id;
                                    if (this.data.contentObject[JSON.stringify(preKey)]) {
                                ...
                                if (!myMessage) {
                                    let regex = new RegExp('<p id="' + reply.paragraph_id.$oid + '">.*?</p>');
                        ...
                        case 'bookmark_updated':
                            let upd_i = this.data.bookmarks[0].children.findIndex((bookmark) => JSON.stringify(reply.bookmark_id) === JSON.stringify(bookmark.data.bookmark_id));
                            this.data.bookmarks[0].children[upd_i].data.name = reply.update.name;
                            break;
                        case 'bookmark_deleted':
                            let del_i = this.data.bookmarks[0].children.findIndex((bookmark) => JSON.stringify(reply.bookmark_id) === JSON.stringify(bookmark.data.bookmark_id));
                        ...
                        case 'note_updated':
                            if (this.data.storyDisplay && this.data.section.data && JSON.stringify(reply.section_id) === JSON.stringify(this.data.section.data.section_id)) {
                                ...
                                if (!myMessage) {
                                    this.data.storyDisplay = this.data.storyDisplay.replace(new RegExp('<p id="' + reply.paragraph_id.$oid + '">(<code>.*?</code>)?(.*?)</p>'), '<p id="' + reply.paragraph_id.$oid + '"><code>' + reply.note + '</code>$2</p>');
                                }
                            }
                            break;
                        case 'note_deleted':
                            if (this.data.storyDisplay && this.data.section.data && JSON.stringify(reply.section_id) === JSON.stringify(this.data.section.data.section_id)) {
                                ...
                                if (!myMessage) {
                                    this.data.storyDisplay = this.data.storyDisplay.replace(new RegExp('<p id="' + reply.paragraph_id.$oid + '"><code>.*?</code>(.*?)</p>'), '<p id="' + reply.paragraph_id.$oid + '">$1</p>');
                                }
                            }
                            break;
                        case 'alias_created':
                            ...
                            if (alias) {
                                ...
                                if (this.data.selectedEntry)
                                    if (JSON.stringify(this.data.selectedEntry.data.id["$oid"]) == JSON.stringify(pid["$oid"])) {
                                        if (this.data.page) {
                                            let idx = this.data.page.aliases.findIndex((ele) => { return JSON.stringify(ele.id) === JSON.stringify(reply.alias_id); });
                        ...
                        case 'passive_link_rejected':
                            ...
                            if (this.data.storyDisplay) {
                                this.data.storyDisplay = this.data.storyDisplay.replace(new RegExp('<a href="(' + reply.passive_link_id.$oid + ')-([a-f0-9]{24})-true" target="_blank" id="true">(.*?)</a>'), '<a href="$1-$2-false" target="_blank" id="false">$3</a>');
                            }
                            break;
                        case 'wiki_created':
                        ...
                        case 'wiki_deleted':
                            break;
                        case 'wiki_updated':
                            let wiki = this.data.selectedEntry;
                        ...
                        case 'segment_added':
                            ...
                            if (this.outgoing['segment' + reply.title]) {
                                let callback = this.outgoing['segment' + reply.title].callback;
                                callback(reply);
                        ...
                        case 'page_added':
                            ...
                            if (this.outgoing['page' + reply.title]) {
                                let callback = this.outgoing['page' + reply.title].callback;
                                callback(reply);
                        ...
                        case 'segment_deleted':
                            ...
                            if (this.outgoing['segment' + reply.identifier.message_id]) {
                                let callback = this.outgoing['segment' + reply.identifier.message_id].callback;
                                callback(reply);
                        ...
                        case 'page_deleted':
                            ...
                            if (this.outgoing['page' + reply.identifier.message_id]) {
                                let callback = this.outgoing['page' + reply.identifier.message_id].callback;
                                callback(reply);
                                delete this.outgoing['page' + reply.identifier.message_id];
                            }
                            break;
                        case 'segment_updated':
                            let seg = this.data.selectedEntry;
                            ...
                            break;
                        case 'page_updated':
                            let page = this.data.selectedEntry;
                        ...
                        case 'got_wiki_segment':
                            this.data.page = JSON.parse(JSON.stringify(reply).replace('template_headings', 'headings'));
                            if (this.outgoing["segment" + reply.identifier.message_id]) {
                                let callback = this.outgoing["segment" + reply.identifier.message_id].callback;
                                callback(reply);
                                delete this.outgoing["segment" + reply.identifier.message_id];
                            ...
                            break;
                        case 'got_wiki_page':
                            this.data.page = this.parser.setPageDisplay(reply, this.data.linkTable, this.data.aliasTable, this.data.passiveLinkTable);
                            ...
                            if (this.outgoing["page" + reply.identifier.message_id]) {
                                ...
                                if (this.outgoing["page" + reply.identifier.message_id].metadata &&
                                    ...
                                    this.data.selectedEntry = this.parser.findPage(this.data.wikiNav[0], this.outgoing["page" + reply.identifier.message_id].metadata.page_id);
                        ...
                        case 'heading_deleted':
                            let head = this.data.selectedEntry;
                        ...
                        case 'got_wiki_segment_hierarchy':
                            ...
                            if (this.data.storyFunction) {
                                let callback = this.data.storyFunction;
                                callback(reply);
                            }
                            break;
                        ...
                        case 'got_paragraph_statistics':
                            ...
                            if (this.outgoing["stats" + reply.identifier.message_id]) {
                                let callback = this.outgoing["stats" + reply.identifier.message_id].callback;
                                callback(reply);
        ...
        this.messages.subscribe((action) => {
            console.log(action);
        });
    }
    refreshStoryInfo() {
    ...
    refreshStoryHierarchy() {
        this.send({ action: 'get_story_hierarchy' }, () => { }, { load: true });
    }
    refreshBookmarks() {
        this.send({ action: 'get_story_bookmarks' }, () => { }, { load: true });
    }
    refreshStoryContent(sectionID = this.data.section.data.section_id, title = this.data.section.data.title) {
        if (!title) {
            let sectionNode = this.parser.findSection(this.data.storyNode[0], JSON.stringify(sectionID));
            if (sectionNode) {
                title = sectionNode.data.title;
            }
        }
        this.data.storyDisplay = '';
        this.send({ action: 'get_section_content', section_id: sectionID }, () => { }, { load: true, title: title, section_id: sectionID });
    }
    refreshWikiInfo() {
        this.send({ action: 'get_wiki_information' }, () => { }, { load: true });
    }
    refreshWikiHierarchy() {
        this.send({ action: 'get_wiki_hierarchy' }, () => { }, { load: true });
    }
    refreshLinkAliasTable() {
        this.send({ action: 'get_wiki_alias_list' }, () => { }, { load: true });
    }
    resetData() {
        this.data = {
            loading: false,
            tooltip: new tooltip_model_1.Tooltip(),
            collaborators: new Array(),
            author: '',
            user: new user_model_1.User(),
            stories: new Array(),
            wikis: new Array(),
            newWiki: new wiki_summary_model_1.WikiSummary(),
            linkTable: new link_table_model_1.LinkTable(),
            aliasTable: new alias_table_model_1.AliasTable(),
            passiveLinkTable: new passive_link_table_model_1.PassiveLinkTable(),
            storyDisplay: '',
            story: new story_model_1.Story(),
            prevSection: new section_model_1.Section(),
            section: new section_model_1.Section(),
            storyNode: new Array(),
            contentObject: new content_object_model_1.ContentObject(),
            lastID: new id_model_1.ID(),
            bookmarks: new Array(),
            statSection: new section_model_1.Section(),
            statSegments: [],
            stats: new stats_model_1.Stats(),
            statsPages: {},
            statsSections: {},
            statsPageFrequency: {},
            wikiNav: [],
            wikiDisplay: '',
            wiki: new wiki_model_1.Wiki(),
            segment: new segment_model_1.Segment(),
            page: new page_model_1.Page(),
            wikiFuctions: new Array(),
            storyFunction: new Function(),
            selectedEntry: {},
            fromContext: Boolean
        };
    }
    send(message, callback = () => { }, metadata = {}) {
...
ApiService = __decorate([
    core_1.Injectable(), 
    __metadata('design:paramtypes', [websocket_service_1.WebSocketService, parser_service_1.ParserService])
], ApiService);
