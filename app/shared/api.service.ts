...
export class ApiService {
  ...
  public uuid: string;
  public local = true;
  public subscribedToWiki = false;
  public subscribedToStory = false;
  ...
  constructor(private socket: WebSocketService, private parser: ParserService) {
    this.resetData();
  }
  ...
  public connect() {
    const url: string = this.local
      ...
      : "wss://inkweaver.plotypus.net:8080/ws";

    this.messages = <Subject<string>>(
      this.socket.connect(url).map((res: MessageEvent) => {
        ...
        if (reply) {
          ...
          const identifier: any = reply.identifier;
          if (identifier) {
            myMessage = identifier.uuid === this.uuid;
            const message_id: number = identifier.message_id;
          ...
          if (reply.event) {
            switch (reply.event) {
              ...
              case "acknowledged":
                ...
                for (const queue of this.queued) {
              ...
              case "story_collaborator_removed":
                index = this.data.collaborators.findIndex((collaborator) => {
                  return (
                    collaborator.value &&
                    JSON.stringify(reply.user_id) ===
                      JSON.stringify(collaborator.value.user_id)
                  );
                });
                if (index > -1) {
              ...
              case "story_collaborator_status_revoked":
                index = this.data.stories.findIndex((story: StorySummary) => {
                  return (
                    JSON.stringify(reply.story_id) ===
                    JSON.stringify(story.story_id)
                  );
                });
                if (index > -1) {
              ...
              case "story_updated":
                ...
                this.data.stories[index].title = reply.update.title;
                break;
              ...
              case "got_section_content":
                if (!metadata.pdf) {
                  ...
                  [this.data.contentObject, this.data.lastID] =
                  ...
                  this.data.section = this.parser.setSection(
                    this.data.storyNode[0],
                    JSON.stringify(metadata.section_id)
                  );
                  this.data.prevSection = this.data.section;
                  ...
                  if (
                    ...
                    this.data.storyDisplay =
                      "<h1>Summary</h1>" + this.data.storyDisplay;
                  } else {
                    this.data.storyDisplay =
                      "<h1>" +
                      metadata.title +
              ...
              case "inner_subsection_added":
                this.parser.findSection(
                  this.data.storyNode[0],
                  JSON.stringify(reply.parent_id),
                  (found: TreeNode) => {
              ...
              case "section_title_updated":
                this.parser.findSection(
                  this.data.storyNode[0],
                  JSON.stringify(reply.section_id),
                  (found: TreeNode) => {
                    const old: string = found.data.title;
                    found.data.title = reply.new_title;
                    if (
                      this.data.storyDisplay &&
                      this.data.section.data &&
                      JSON.stringify(this.data.section.data.section_id) ===
                        JSON.stringify(found.data.section_id)
                    ) {
              ...
              case "section_deleted":
                this.parser.findSection(
                  ...
                  JSON.stringify(reply.section_id),
                  (found: TreeNode) => {
                    index = found.parent.children.indexOf(found);
              ...
              case "subsection_moved_as_inner":
                const moved: TreeNode = this.parser.findSection(
                  this.data.storyNode[0],
                  JSON.stringify(reply.section_id),
                  (found: TreeNode) => {
                    index = found.parent.children.indexOf(found);
                ...
                this.parser.findSection(
                  this.data.storyNode[0],
                  JSON.stringify(reply.to_parent_id),
                  (found: TreeNode) => {
                    moved.parent = found;
              ...
              case "paragraph_added":
                if (
                  ...
                  const p: Paragraph = {
                    ...
                    text: reply.text,
                    note: null,
                    preceding_paragraph_id: new ID(),
                    succeeding_paragraph_id: reply.succeeding_paragraph_id,
                  ...
                  if (p.succeeding_paragraph_id) {
                    ...
                    if (this.data.contentObject[JSON.stringify(postKey)]) {
                      preKey =
                        this.data.contentObject[JSON.stringify(postKey)]
                  ...
                  if (this.data.contentObject[JSON.stringify(preKey)]) {
                    ...
                    ].succeeding_paragraph_id = p.paragraph_id;
                    p.preceding_paragraph_id = preKey;
                  }
                  if (this.data.contentObject[JSON.stringify(postKey)]) {
                    this.data.contentObject[
                      JSON.stringify(postKey)
                    ].preceding_paragraph_id = p.paragraph_id;
                  }

                  // Add/update the paragraph in the display
                  const pString: string = this.parser.setParagraph(p);
                  this.data.story.position_context = {
              ...
              case "paragraph_updated":
                if (
                  ...
                  // Update the content object
                  const p: Paragraph =
                    this.data.contentObject[JSON.stringify(reply.paragraph_id)];
              ...
              case "paragraph_deleted":
                if (
                  ...
                  if (
                    ...
                    const p: Paragraph =
                      this.data.contentObject[
                        JSON.stringify(reply.paragraph_id)
                      ];
                    const preKey: ID = p.preceding_paragraph_id;
                    const postKey: ID = p.succeeding_paragraph_id;
                    if (this.data.contentObject[JSON.stringify(preKey)]) {
                      this.data.contentObject[
                        JSON.stringify(preKey)
                      ].succeeding_paragraph_id = postKey;
                      this.data.story.position_context = {
                        cursor: preKey.$oid,
                      };
                    }
                    if (this.data.contentObject[JSON.stringify(postKey)]) {
                      this.data.contentObject[
                        JSON.stringify(postKey)
                      ].preceding_paragraph_id = preKey;
                      this.data.story.position_context = {
                        cursor: postKey.$oid,
                      };
                    }
                    delete this.data.contentObject[
                      JSON.stringify(reply.paragraph_id)
                    ];
                  }
              ...
              case "got_story_bookmarks":
                this.data.bookmarks = [
                  {
                    data: { name: "Bookmarks", bookmark_id: { $oid: null } },
                    expanded: true,
                    children: [],
                  },
                ];
                for (const bookmark of reply.bookmarks) {
                  bookmark.index = this.data.bookmarks[0].children.length;
                  this.data.bookmarks[0].children.push({
                    data: bookmark,
                    parent: this.data.bookmarks[0],
                  });
                }
                break;
              case "bookmark_added":
                reply.index = reply.index
                  ? reply.index
                  : this.data.bookmarks[0].children.length;
                this.data.bookmarks[0].children.splice(reply.index, 0, {
                  data: reply,
                  parent: this.data.bookmarks[0],
                });
                break;
              case "bookmark_updated":
                index = this.data.bookmarks[0].children.findIndex(
                  (bookmark: TreeNode) =>
                    JSON.stringify(reply.bookmark_id) ===
                    JSON.stringify(bookmark.data.bookmark_id)
                );
                this.data.bookmarks[0].children[index].data.name =
                  reply.update.name;
                break;
              ...
              case "alias_created":
                this.data.aliasTable[JSON.stringify(reply.alias_id)] = {
                  ...
                  alias_name: reply.alias_name,
                };
                if (this.data.selectedEntry) {
                  if (
                    JSON.stringify(this.data.selectedEntry.data.id["$oid"]) ===
                    JSON.stringify(reply.page_id["$oid"])
                  ) {
              ...
              case "alias_updated":
                ...
                if (alias) {
                  ...
                  const pid = alias.page_id;
                  if (this.data.selectedEntry) {
                    if (
                      JSON.stringify(
                        this.data.selectedEntry.data.id["$oid"]
                      ) === JSON.stringify(pid["$oid"])
                    ) {
                      // need to find index fo updated alias
                      if (this.data.page) {
                        index = this.data.page.aliases.findIndex((ele: any) => {
                          return (
                            JSON.stringify(ele.id) ===
                            JSON.stringify(reply.alias_id)
                          );
                        });
                        this.data.page.aliases[index].name = reply.new_name;
                      }
                    }
                  }
                }
                  ...
                  if (this.data.selectedEntry) {
                    if (
                      ...
                      if (this.data.page) {
                        ...
                        this.data.page.aliases.splice(index, 1);
                      }
                    }
                  }
                }
              ...
              case "link_deleted":
                if (this.data.linkTable[JSON.stringify(reply.link_id)]) {
                  this.data.linkTable[JSON.stringify(reply.link_id)].deleted =
                    true;
                }
                break;
              case "passive_link_created":
                this.data.passiveLinkTable[
                  JSON.stringify(reply.passive_link_id)
                ] = {
                  alias_id: reply.alias_id,
                  pending: true,
                  deleted: false,
              ...
              case "page_added":
                ...
                this.data.selectedEntry = this.parser.findPage(
                  this.data.wikiNav[0],
                  reply.page_id
                );
                if (this.outgoing["page" + reply.title]) {
                  callback = this.outgoing["page" + reply.title].callback;
                  callback(reply);
              ...
              case "segment_deleted":
                ...
                if (this.outgoing["segment" + reply.identifier.message_id]) {
                  callback =
                    this.outgoing["segment" + reply.identifier.message_id]
                      .callback;
                  callback(reply);
              ...
              case "page_deleted":
                ...
                if (this.outgoing["page" + reply.identifier.message_id]) {
                  callback =
                    this.outgoing["page" + reply.identifier.message_id]
                      .callback;
                  callback(reply);
              ...
              case "page_updated":
                ...
                if (
                  JSON.stringify(reply.page_id) === JSON.stringify(page.data.id)
                ) {
                  this.data.page.title = reply.update["title"];
                  page.label = reply.update["title"];
                  page.data.title = reply.update["title"];
                } else {
                  page = this.parser.findSegment(
                    this.data.wikiNav[0],
                    reply.page_id
                  );
                  page.label = reply.update["title"];
              ...
              case "got_wiki_segment":
                this.data.page = JSON.parse(
                  JSON.stringify(reply).replace("template_headings", "headings")
                );
                if (this.outgoing["segment" + reply.identifier.message_id]) {
                  callback =
                    this.outgoing["segment" + reply.identifier.message_id]
              ...
              case "got_wiki_page":
                ...
                if (reply.headings && reply.headings[0]) {
                  this.data.tooltip.text +=
                    "<br/><u>" +
                    reply.headings[0].title +
                    "</u><br/>" +
                    reply.headings[0].text;
                }
                if (this.outgoing["page" + reply.identifier.message_id]) {
                  callback =
                    this.outgoing["page" + reply.identifier.message_id]
                  ...
                  if (
                    this.outgoing["page" + reply.identifier.message_id]
                      .metadata &&
                    this.outgoing[
                      "page" + reply.identifier.message_id
                    ].metadata.hasOwnProperty("page_id")
                  ) {
                    this.data.selectedEntry = this.parser.findPage(
                      this.data.wikiNav[0],
                      this.outgoing["page" + reply.identifier.message_id]
              ...
              case "move_segment":
                if (!myMessage) {
                  const sid = reply.segment_id;
                  const to_pid = reply.to_parent_id;

                  const curr_node = this.parser.findSegment(
                    this.data.wikiNav[0],
                    sid
                  );
                  const parent_node = this.parser.findSegment(
                    this.data.wikiNav[0],
                    to_pid
                  );

                  // remove from current location
                  index = curr_node.parent.children.indexOf(curr_node);
                  curr_node.parent.children.splice(index, 1);

                  parent_node.children.splice(reply.to_index, 0, curr_node);
              ...
              case "page_moved":
                if (!myMessage) {
                  const pid = reply.page_id;
                  const to_pid = reply.to_parent_id;

                  const curr_node = this.parser.findPage(
                    this.data.wikiNav[0],
                    pid
                  );
                  const parent_node = this.parser.findPage(
                    this.data.wikiNav[0],
                    to_pid
                  );
              ...
              case "template_heading_deleted":
                const t_head = this.data.selectedEntry as TreeNode;
                if (
                  !myMessage &&
                  JSON.stringify(reply.segment_id) ===
                    JSON.stringify(t_head.data.id)
                ) {
                  callback = this.data.wikiFuctions[0];
                  callback(reply);
                }
                break;
              case "heading_added":
              ...
              case "heading_deleted":
                const head = this.data.selectedEntry as TreeNode;
                if (
                  !myMessage &&
                  JSON.stringify(reply.page_id) === JSON.stringify(head.data.id)
                ) {
                  callback = this.data.wikiFuctions[0];
                  callback(reply);
                }
              ...
              case "got_wiki_segment_hierarchy":
                ...
                const result = this.parser.parseWiki(
                  reply.hierarchy,
                ...
                if (this.data.storyFunction) {
                  callback = this.data.storyFunction;
                  callback(reply);
                }
              ...
              case "got_wiki_alias_list":
                const temp = this.parser.parseLinkTable(reply.alias_list);
              ...
              case "got_page_frequencies":
                this.data.statsPageFrequency = this.parser.parsePageFrequency(
                  reply.pages,
                  this.data.statsPages,
                  this.data.statsSections
                );
                break;
              ...
              case "got_paragraph_statistics":
                ...
                if (this.outgoing["stats" + reply.identifier.message_id]) {
                  callback =
                    this.outgoing["stats" + reply.identifier.message_id]
                      .callback;
                  callback(reply);
  ...
  public refreshStoryContent(
    ...
    if (!title) {
      const sectionNode: TreeNode = this.parser.findSection(
        this.data.storyNode[0],
    ...
    this.data.storyDisplay = "";
    this.send(
      { action: "get_section_content", section_id: sectionID },
      () => {},
