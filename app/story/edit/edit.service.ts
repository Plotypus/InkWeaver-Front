  ...
  public compare(
    ...
    for (const id in obj1) {
      if (!obj2[id]) {
        ...
        for (const link in obj1[id].links) {
          if (obj1[id].links.hasOwnProperty(link)) {
            const linkObj: Link = this.apiService.data.linkTable[link];
            if (linkObj) {
              const aliasID: ID = linkObj.alias_id;
              const alias: Alias =
                this.apiService.data.aliasTable[JSON.stringify(aliasID)];
              deleted[link] =
                "{#|" +
        ...
        for (const passive in obj1[id].passiveLinks) {
          if (obj1[id].passiveLinks.hasOwnProperty(passive)) {
            const linkObj: PassiveLink =
              this.apiService.data.passiveLinkTable[passive];
            if (linkObj) {
              const aliasID: ID = linkObj.alias_id;
              const alias: Alias =
                this.apiService.data.aliasTable[JSON.stringify(aliasID)];
    ...
    for (const id in obj2) {
      if (obj2.hasOwnProperty(id)) {
        const edit: boolean = obj1[id] && obj1[id].text !== obj2[id].text;
        ...
        for (const dLink in deleted) {
          if (deleted.hasOwnProperty(dLink)) {
        ...
        if (id.startsWith("new")) {
          ...
          this.addParagraph(
            ...
            sectionID,
            obj2[id].text,
            obj2[id].succeeding_paragraph_id
          );
        } else {
